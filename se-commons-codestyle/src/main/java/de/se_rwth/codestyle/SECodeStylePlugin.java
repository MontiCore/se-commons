...
public class SECodeStylePlugin implements Plugin<Project> {
  ...
  public void apply(@Nonnull Project project) {
    ...
    if (!editorConfig.exists()) {
      try {
        try (InputStream url = getClass().getClassLoader().getResourceAsStream("se-codestyle/.editorconfig")) {
          Files.copy(Objects.requireNonNull(url), editorConfig.toPath(), StandardCopyOption.REPLACE_EXISTING);
        }
      } catch (IOException e) {
